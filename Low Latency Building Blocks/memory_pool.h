...
namespace Common {
    ...
    // Memory pool class for managing memory allocation and deallocation
    template<typename T>
    class MemPool {
    public:
        // Constructor: Initializes the memory pool with a specified number of elements
        explicit MemPool(std::size_t num_elems) :
            store_(num_elems, { T(), true }) {}

        // Allocates memory for a new object of type T
        template<typename... Args>
        T* allocate(Args... args) noexcept {
            auto obj_block = &(store_[next_free_index_]);
            obj_block->is_free_ = false; // Mark the block as not free
            return new(&(obj_block->object_)) T(args...); // Placement new
        }

        // Deallocates memory for an object of type T
        auto deallocate(const T* elem) noexcept {
            auto elem_index = elem - &store_[0].object_;
            store_[elem_index].is_free_ = true; // Mark the block as free
        }

    private:
        // Object block representing a memory block with an object of type T
        struct ObjectBlock {
            T object_;
            bool is_free_ = true; // Flag indicating whether the block is free
        };

        std::vector<ObjectBlock> store_; // Vector storing ObjectBlock elements
        size_t next_free_index_ = 0; // Index of the next free memory block
    };
